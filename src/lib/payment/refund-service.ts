 ...
 */
export async function executeChannelRefund(
  refundId: string,
    ...
    refundResult = await resolved.provider.refund({
      ...
      notifyUrl: order.channel.startsWith("WECHAT")
        ...
        : order.channel.startsWith("UNIONPAY")
          ? `${resolveBaseUrl()}/api/pay/unionpay/notify`
          : undefined,
    });
  } catch (error) {
    refundResult = {
      success: false,
      error: `渠道退款调用异常: ${(error as Error).message}`,
    };
  }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.order.update({
        ...
        data: {
          ...
          status:
            nextRefundAmount >= Number(currentOrder.amount)
              ? "REFUNDED"
              : "PARTIALLY_REFUNDED",
        },
      });
  ...
  return {
    ...
    orderUpdated: false,
    error:
      queryStatus === "UNKNOWN" ? "渠道退款终态未确认，需查单补偿" : undefined,
  };
}

/** 对 PROCESSING 退款执行官方查单补偿，仅官方确认 SUCCESS 后更新订单。 */
export async function syncChannelRefund(
  refundId: string,
): Promise<RefundExecution> {
  const refund = await prisma.refund.findUnique({
    where: { id: refundId },
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const nextRefundAmount =
      Number(currentOrder.refundAmount) + Number(refund.amount);
    await tx.order.update({
      where: { id: refund.order.id },
      data: {
